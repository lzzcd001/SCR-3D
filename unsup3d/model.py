...
class Unsup3D():
    def __init__(self, cfgs, dense_order=None, adjacency=None):
        ...
        assert not (dense_order is not None and adjacency is not None)
        ...
        if dense_order is not None:
            ...
            if self.use_concat:
                # self.factor_mlp_list = [networks.FactorMLP(256*4, 256) for _ in range(4)]
            ...
            self.independent = False
            self.adjacency = torch.tensor(adjacency).float().view(4,4)
            if self.use_concat:
                self.factor_mlp_list = []
                for factor in ['depth', 'albedo', 'light', 'view']:
                    factor_mlp = networks.FactorMLP(256*4, 256)
                    setattr(self, 'factor_mlpnet_' + factor, factor_mlp)
                    self.factor_mlp_list.append(factor_mlp)
        print("independent", self.independent)
    ...
    def forward(self, input):
        ...
        if not self.independent:
            ...
            bs, _, _, _, _ = concat_emb.size()
            adjacency = self.adjacency.to(concat_emb.device) + torch.eye(4, device=concat_emb.device)
            
            if self.use_concat:
                for i, factor in enumerate(['depth', 'albedo', 'light', 'view']):
                    concat_input = concat_emb * adjacency[i][None, None, :, None, None].abs()
                    emb_dict[factor] = self.factor_mlp_list[i](concat_input.view(bs, -1, 1, 1))
            else:
                for i, factor in enumerate(['depth', 'albedo', 'light', 'view']):
                    emb_dict[factor] = (concat_emb * adjacency[i][None, None, :, None, None].abs()).sum(2)
        ...
        if self.load_gt_depth:
            ...
            metrics['NorErr_masked'] = self.acc_normal_masked.mean()
            metrics['Combined'] = (
                1.0 * metrics['Image_masked'] 
                + 1.0 * metrics['Perc_masked'] 
                + 0.1 * metrics['NorErr_masked']
            )

        return metrics


    def forward_interp(self, input, interp_coeff=1.0):
        """Feedforward once."""
        if self.load_gt_depth:
            input, depth_gt = input
        self.input_im = input.to(self.device) *2.-1.
        ...
        depth_int = emb_dict['depth'].clone()
        albedo_int = emb_dict['albedo'].clone()
        light_int = emb_dict['light'].clone()
        view_int = emb_dict['view'].clone()

        emb_dict['depth'][:b//2] = depth_int[:b//2] * 1 + (depth_emb[b//2:, :, 0] - depth_emb[:b//2, :, 0]) * (interp_coeff)
        emb_dict['albedo'][:b//2] = albedo_int[:b//2] * 1 + (albedo_emb[b//2:, :, 0] - albedo_emb[:b//2, :, 0]) * (interp_coeff)
        emb_dict['light'][:b//2] = light_int[:b//2] * 1 + (light_emb[b//2:, :, 0] - light_emb[:b//2, :, 0]) * (interp_coeff)
        emb_dict['view'][:b//2] = view_int[:b//2] * 1 + (view_emb[b//2:, :, 0] - view_emb[:b//2, :, 0]) * (interp_coeff)



        emb_dict['depth'][b//2:] = depth_int[:b//2] * interp_coeff + depth_int[b//2:] * (1 - interp_coeff)
        emb_dict['albedo'][b//2:] = albedo_int[:b//2] * interp_coeff + albedo_int[b//2:] * (1 - interp_coeff)
        emb_dict['light'][b//2:] = light_int[:b//2] * interp_coeff + light_int[b//2:] * (1 - interp_coeff)
        emb_dict['view'][b//2:] = view_int[:b//2] * interp_coeff + view_int[b//2:] * (1 - interp_coeff)
    ...
    def save_results(self, save_dir, no_viz=False):
        ...
        with torch.no_grad():
            ...
            # canon_shape_rotate = canon_shape_rotate.clamp(-1,1).detach().cpu()
    ...
    def gen_viz(self, logger, max_bs=25):
        total_iter = 0
        b, c, h, w = self.input_im.shape
        b0 = min(max_bs, b)
