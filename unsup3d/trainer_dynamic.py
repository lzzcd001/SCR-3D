...
class DynamicTrainer():
    def __init__(self, cfgs, model, dense_order=None, seed=0):
        ...
        self.checkpoint_dir += f'_dynemb_{self.model.dynamic_emb}'
        if self.test_result_dir is not None:
            self.test_result_dir += f'_dynemb_{self.model.dynamic_emb}'
    ...
    def run_epoch(self, loader, epoch=0, is_validation=False, is_test=False, no_viz=False):
        ...
        for iter, input in enumerate(loader):
            ...
            if (self.model.dynamic_emb == 'attention' and self.global_iter >= self.model.lam_warmup
    ...
    def run_iter(self, loader, epoch=0, is_validation=False, is_test=False):
        """Run one iter."""
        is_train = not is_validation and not is_test
        metrics = self.make_metrics()
