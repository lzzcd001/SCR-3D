...
class DartsUnsup3D(nn.Module):
    def __init__(self, cfgs, dense_order=None):
        super(DartsUnsup3D, self).__init__()
        ...
        self.darts_coeff = cfgs.get('darts_coeff', 10.0)
    ...
    def backward_meta(self, darts_train_loader, darts_val_loader):
        ...
        with higher.innerloop_ctx(self, getattr(self, 'global_optimizer')) as (fmodel, diffopt):
            ...
            if self.load_gt_depth:
                ...
                gt_depth = None
            meta_loss = fmodel.forward_inner(darts_input, adjacency, depth_gt=gt_depth)[self.darts_obj]

            # loss_adj = self.DAGLoss(adjacency.abs())
            # loss_adj_coeff = 1.0
            # loss_adj_coeff = self.lam_adj
            # meta_loss = self.darts_coeff * meta_loss + loss_adj_coeff * loss_adj
            # meta_loss.backward()
    
            meta_loss = self.darts_coeff * meta_loss
    ...
    def forward_inner(self, input, adjacency, depth_gt=None):
        ...
        if depth_gt is not None:
            ...
            loss_l1_masked = self.photometric_loss(recon_im[:b], input_im, mask=mask[:b].unsqueeze(1))
            ...
            loss_dict['Image_masked'] = loss_l1_masked.mean()
