...
class DynamicUnsup3D(nn.Module):
    def __init__(self, cfgs, dense_order=None):
        super(DynamicUnsup3D, self).__init__()
        ...
        ### adj related
        self.lam_adj = cfgs.get('lam_adj', 10.0)
        ...
        ###
        if self.dynamic_emb == 'attention':
            self.K = nn.Parameter(torch.randn(256, 256))
            self.Q = nn.Parameter(torch.randn(256, 256))

        if dense_order is not None:
            adjacency = torch.zeros(4, 4).float()
            for i, indx in enumerate(dense_order):
                if i == 0:
                    continue
                for j in range(i):
                    adjacency[indx, dense_order[j]] = 1.0
            self.adjacency_mask = nn.Parameter(adjacency, requires_grad=False)
            print('adj mask\n\n', adjacency)
            self.no_dag_loss = True
            # raise
        else:
        ...
        for net_name in self.network_names:
            params.extend(list(getattr(self, net_name).parameters()))

        print(f"Parameter Count: {sum(p.numel() for p in params if p.requires_grad)}")
        raise
    ...
    def backward(self):
        ...
        if self.dynamic_emb == 'attention' and not self.no_dag_loss:
            # loss_adj = self.batch_DAGLoss(self.saved_adj)
            loss_adj = self.dag_loss
            (self.loss_total + self.lam_adj * loss_adj).backward()
        else:
            self.loss_total.backward()
        self.global_optimizer.step()

    def batch_DAGLoss(self, adj):
        m = 4
        # loss = (torch.vmap(torch.trace)(torch.matrix_power(torch.eye(m).to(adj.device) + adj.pow(2) / m, m)) - m).pow(2)
        loss = ((torch.matrix_power(torch.eye(m).to(adj.device) + adj.pow(2) / m, m)).diagonal(offset=0, dim1=-1, dim2=-2).sum(-1) - m).pow(2)
    ...
    def forward(self, input):
        ...
        if self.dynamic_emb == 'dense':
            normed_concat_emb = concat_emb.permute(0, 2, 1, 3, 4)
            adjacency = torch.sigmoid(torch.einsum('nxacd, nyacd->nxy', normed_concat_emb, normed_concat_emb) / np.sqrt(ch * hz * wz))
            # adjacency_aug = adjacency.to(concat_emb.device).abs() * self.adjacency_mask.to(concat_emb.device) + torch.eye(4, device=concat_emb.device)
        elif self.dynamic_emb == 'attention':
            normed_concat_emb = concat_emb.permute(0, 2, 1, 3, 4) / np.sqrt(ch * hz * wz)
            key = torch.einsum('nxacd,ce->nxaed', normed_concat_emb, self.K.to(self.input_im.device))
            query = torch.einsum('nxacd,ce->nxaed', normed_concat_emb, self.Q.to(self.input_im.device))
            adjacency = torch.sigmoid(torch.einsum('nxacd, nyacd->nxy', key, query) / np.sqrt(ch * hz * wz))
            # adjacency_aug = adjacency.to(concat_emb.device).abs() * (1.0 - torch.eye(4, device=concat_emb.device)) + torch.eye(4, device=concat_emb.device)
        elif self.dynamic_emb == 'cos':
            normed_concat_emb = concat_emb.permute(0, 2, 1, 3, 4)
            normed_concat_emb = normed_concat_emb  / torch.sqrt(normed_concat_emb.pow(2).sum(dim=[2,3,4], keepdim=True) + 1e-8)
            adjacency = torch.einsum('nxacd, nyacd->nxy', normed_concat_emb, normed_concat_emb) * (1.0 - torch.eye(4, device=concat_emb.device))
            # adjacency_aug = adjacency.to(concat_emb.device) + torch.eye(4, device=concat_emb.device)
        

        if self.adjacency_mask is not None:
            self.adjacency = adjacency.to(concat_emb.device).abs() * self.adjacency_mask.to(concat_emb.device)
        ...
        if self.dynamic_emb == 'dense' or self.dynamic_emb == 'cos':
            metrics = {'loss': self.loss_total}
        else:
            dag_loss = self.dag_loss = self.batch_DAGLoss(adjacency.abs())
    ...
    def save_results(self, save_dir, no_viz=False):
        b, c, h, w = self.input_im.shape
        ...
        with torch.no_grad():
            ...
            # canon_shape_im = (self.canon_normal * front_light.view(1,1,1,3)).sum(3)
            # canon_shape_im = canon_shape_im.clamp(min=0).unsqueeze(1)
            # canon_shape_im = canon_shape_im.repeat(1,3,1,1) * 0.7
            # canon_shape_rotate = self.renderer.render_yaw(canon_shape_im[:b], self.canon_depth[:b], v_before=v0, maxr=90, nsample=5)  # (B,T,C,H,W)

            # canon_im_rotate_few = self.renderer.render_yaw(self.canon_im[:b], self.canon_depth[:b], v_before=v0, maxr=90, nsample=5)  # (B,T,C,H,W)
            canon_im_rotate_few = self.renderer.render_yaw(self.canon_im[:b], self.canon_depth[:b], v_before=v0, maxr=90, nsample=7)  # (B,T,C,H,W)
            canon_im_rotate_few = canon_im_rotate_few.clamp(-1,1).detach().cpu() /2+0.5
        ...
        if not no_viz:
            ...
            # for i in range(canon_shape_rotate.size(1)):
            #     utils.save_images(save_dir, canon_shape_rotate[:, i, :, :, :].cpu().detach().numpy(), suffix=f'shape_image_{i}', sep_folder=sep_folder)
            for i in range(canon_im_rotate_few.size(1)):
                utils.save_images(save_dir, canon_im_rotate_few[:, i, :, :, :].cpu().detach().numpy(), suffix=f'colored_shape_image_{i}', sep_folder=sep_folder)
            ...
            # # print(canon_light.shape)
            # # print(view.shape)
            # # print(self.adjacency[:b].cpu().view(b, -1).numpy().shape)
            # # print(self.adjacency[:b].cpu().view(b, -1).numpy()[0])
            # utils.save_txt(save_dir, self.adjacency[:b].cpu().view(b, -1).numpy(), suffix='dag', sep_folder=sep_folder)
            ...
            # # utils.save_videos(save_dir, canon_shape_rotate_grid, suffix='shape_video', sep_folder=sep_folder, cycle=True)
